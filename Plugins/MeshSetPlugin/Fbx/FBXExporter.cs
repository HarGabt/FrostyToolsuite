...
namespace MeshSetPlugin
{
    public class FBXExporter
    {
        ...
        private FbxNode FBXCreateSkeleton(FbxScene scene, dynamic meshAsset, string skeleton, ref List<FbxNode> boneNodes)
        {
            ...
            if (ProfilesLibrary.IsLoaded(ProfileVersion.NeedForSpeedHeat, ProfileVersion.NeedForSpeedUnbound)) //Was initally made for Heat, I added Unbound here. Report if broken pls
            {
            ...
            for (int boneIdx = 0; boneIdx < m_boneCount; boneIdx++)
            {
                ...
                //RiggingOffsetJointComponent
                if (ProfilesLibrary.IsLoaded(ProfileVersion.NeedForSpeedHeat, ProfileVersion.NeedForSpeedUnbound) && jointOffsets.Count > 0)
                {
